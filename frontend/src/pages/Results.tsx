...
import { Download, Maximize2, Users, BarChart3, X, ArrowRight, Loader2, User, Info } from 'lucide-react';
...
const Results: React.FC = () => {
    ...
    useEffect(() => {
        if (!data && id) {
            axios.get(`${API_BASE_URL}/results/${id}`)
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            try {
                const imgUrl = `${API_BASE_URL}${data.image_url}`;
    ...
    return (
        <Layout title={`raati.ai — Evaluation Report ${data.submitter_name ? `for ${data.submitter_name}` : ''}`}>
            <div ref={reportRef} className="space-y-5 transition-colors duration-300">
                ...
                <div className={`${cardBg} border rounded-xl shadow-sm overflow-hidden`}>
                    {/* ─── HEADER ─── */}
                    <div className={`px-5 py-3 flex items-center gap-3 border-b ${divider}`}>
                        <h2 className={`text-xs font-black uppercase tracking-widest ${txt}`}>Assessment Summary</h2>
                        <div className="flex-1" />
                        {data.submitter_name && (
                            <span className={`flex items-center gap-1.5 text-[10px] font-semibold ${sub}`}>
                                <User size={11} strokeWidth={2.5} className="opacity-50" />
                                {data.submitter_name.trim()}
                            </span>
                        )}
                        <span className={`text-[9px] font-mono px-2 py-0.5 rounded ${dark ? 'bg-gray-800 text-gray-500' : 'bg-gray-100 text-gray-400'}`}>
                            {data.id?.slice(0, 8)}
                        </span>
                        <button onClick={handleDownloadPDF} disabled={pdfLoading}
                            className={`px-3 py-1.5 border rounded-lg font-medium text-[10px] flex items-center gap-1.5 transition-colors ${dark ? 'border-blue-500/50 text-blue-400 hover:bg-blue-500/10' : 'border-[#1a237e] text-[#1a237e] hover:bg-blue-50'} ${pdfLoading ? 'opacity-50 cursor-wait' : ''}`}>
                            {pdfLoading ? <Loader2 size={12} className="animate-spin" /> : <Download size={12} />}
                            {pdfLoading ? 'Generating...' : 'PDF'}
                        </button>
                    </div>

                    {/* ─── MAIN 3-COLUMN GRID — fixed height, zero whitespace ─── */}
                    {(() => {
                        const dims = [
                            { label: 'Creativity',   score: data.creativity_score,                    reasoning: data.creativity_reasoning },
                            { label: 'Originality',  score: data.originality_score,                   reasoning: data.originality_reasoning },
                            { label: 'Usefulness',   score: data.usefulness_relevance_score,           reasoning: data.usefulness_relevance_reasoning },
                            { label: 'Clarity',      score: data.clarity_score,                       reasoning: data.clarity_reasoning },
                            { label: 'Detail',       score: data.level_of_detail_elaboration_score,   reasoning: data.level_of_detail_elaboration_reasoning },
                            { label: 'Feasibility',  score: data.feasibility_score,                   reasoning: data.feasibility_reasoning },
                        ] as { label: string; score: number; reasoning: string }[];
                        const sorted = [...dims].sort((a, b) => b.score - a.score);
                        const best = sorted[0], weakest = sorted[sorted.length - 1];
                        const scoreCol = (s: number) => s >= 4.5 ? '#10b981' : s >= 3.5 ? '#3b82f6' : s >= 2.5 ? '#f59e0b' : '#ef4444';
                        const overallCol = scoreCol(data.overall_score);

                        return (
                            <>
                                {/* CSS Grid: image | chart | dims — all equal height */}
                                <div
                                    className={`hidden lg:grid border-b ${divider}`}
                                    style={{ gridTemplateColumns: '5fr 6fr 5fr', height: '20rem' }}
                                >
                                    {/* — Col 1: Submission image — */}
                                    <div
                                        className={`relative overflow-hidden border-r ${divider} cursor-pointer group/img`}
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-black/30 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                                            <div className="bg-white/20 backdrop-blur-sm p-2.5 rounded-full"><Maximize2 size={16} className="text-white" /></div>
                                        </div>
                                        {/* Bottom: label + description */}
                                        <div className={`absolute inset-x-0 bottom-0 px-3 pt-10 pb-3 ${dark ? 'bg-gradient-to-t from-black/90 via-black/50 to-transparent' : 'bg-gradient-to-t from-white/95 via-white/60 to-transparent'}`}>
                                            <div className={`text-[7px] font-bold uppercase tracking-widest mb-1 ${dark ? 'text-gray-500' : 'text-gray-400'}`}>Submission</div>
                                            <p className={`text-[9px] leading-relaxed line-clamp-3 ${dark ? 'text-gray-300' : 'text-gray-600'}`}>{data.description}</p>
                                        </div>
                                    </div>

                                    {/* — Col 2: Radar chart — */}
                                    <div className={`flex flex-col border-r ${divider}`} style={{ padding: '12px 16px 8px' }}>
                                        <div className="flex items-center justify-between mb-0.5">
                                            <span className={`text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 ${sub2}`}>
                                                <Users size={10} className="text-blue-500" /> Panel Consensus
                                            </span>
                                            <div className="flex items-center gap-2">
                                                <span className="text-[11px] font-black" style={{ color: overallCol }}>
                                                    {data.overall_score}<span className={`text-[8px] font-semibold ${sub2}`}>/5</span>
                                                </span>
                                                <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full border ${dark ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                                                    {totalResults} evals
                                                </span>
                                            </div>
                                        </div>
                                        <div className="flex-1 min-h-0">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <RadarChart cx="50%" cy="50%" outerRadius="78%" data={chartData}>
                                                    <PolarGrid stroke={dark ? '#1f2937' : '#e5e7eb'} />
                                                    <PolarAngleAxis dataKey="subject" tick={{ fill: dark ? '#9ca3af' : '#6b7280', fontSize: 10, fontWeight: 600 }} />
                                                    <PolarRadiusAxis angle={30} domain={[0, 5]} tick={false} axisLine={false} />
                                                    <Radar name="Score" dataKey="A" stroke="#3b82f6" strokeWidth={2.5} fill="#3b82f6" fillOpacity={0.15} />
                                                    <Radar name="Max" dataKey="A" stroke="none" fill="none" />
                                                </RadarChart>
                                            </ResponsiveContainer>
                                        </div>
                                    </div>

                                    {/* — Col 3: Dimension scores — */}
                                    <div className="flex flex-col px-4 py-3">
                                        <div className={`text-[8px] font-bold uppercase tracking-widest mb-1 ${sub2}`}>Dimension Breakdown</div>
                                        <div className="flex-1 flex flex-col justify-between">
                                            {dims.map((dim) => {
                                                const col = scoreCol(dim.score);
                                                const pct = (dim.score / 5) * 100;
                                                return (
                                                    <div key={dim.label} className="group/d relative">
                                                        <div className="flex items-center gap-2">
                                                            <span className={`text-[10px] font-semibold w-[4.2rem] flex-shrink-0 truncate ${dark ? 'text-gray-300' : 'text-gray-600'}`}>{dim.label}</span>
                                                            {/* Continuous progress bar — cleaner than blocks */}
                                                            ...
                                                            <span className="text-[10px] font-black w-6 text-right flex-shrink-0" style={{ color: col }}>{dim.score}</span>
                                                        </div>
                                                        {/* Tooltip */}
                                                        <div className={`absolute right-0 bottom-full mb-1.5 w-56 p-2.5 text-[9px] leading-relaxed rounded-lg shadow-xl border z-[300] pointer-events-none opacity-0 group-hover/d:opacity-100 transition-opacity ${dark ? 'bg-gray-900 text-gray-300 border-gray-700' : 'bg-white text-gray-700 border-gray-200'}`}>
                                                            <span className="font-bold" style={{ color: col }}>{dim.label} {dim.score}/5 — </span>{dim.reasoning}
                                                        </div>
                                                    </div>
                                                );
                                            })}
                                ...
                                <div className={`lg:hidden flex flex-col border-b ${divider}`}>
                                    <div className={`relative cursor-pointer aspect-video overflow-hidden ${dark ? 'bg-gray-900' : 'bg-gray-50'}`} onClick={() => setIsModalOpen(true)}>
                                        <img src={`${API_BASE_URL}${data.image_url}`} alt="Sketch" className="w-full h-full object-contain" crossOrigin="anonymous" />
                                    </div>
                                    <div className="p-4">
                                        <p className={`text-[10px] leading-relaxed ${sub}`}>{data.description}</p>
                                    </div>
                                    <div className="h-48 px-4">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
                                                <PolarGrid stroke={dark ? '#374151' : '#e5e7eb'} />
                                                <PolarAngleAxis dataKey="subject" tick={{ fill: dark ? '#9ca3af' : '#6b7280', fontSize: 9 }} />
                                                <PolarRadiusAxis angle={30} domain={[0, 5]} tick={false} axisLine={false} />
                                        ...
                                        {dims.map(dim => {
                                            ...
                                            return (
                                                <div key={dim.label} className="flex items-center gap-2">
                                                    <span className={`text-[10px] font-semibold w-16 truncate ${dark ? 'text-gray-300' : 'text-gray-600'}`}>{dim.label}</span>
                                                    <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${dark ? 'bg-gray-800' : 'bg-gray-100'}`}>
                                                        <div className="h-full rounded-full" style={{ width: `${(dim.score/5)*100}%`, backgroundColor: col }} />
                                                    </div>
                                                    <span className="text-[10px] font-black w-5 text-right" style={{ color: col }}>{dim.score}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                ...
                                {/* ─── FOOTER: Peak | Overview | Pivot | Next Step | Focus ─── */}
                                <div className={`grid grid-cols-1 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x ${dark ? 'divide-gray-800/60' : 'divide-gray-100'}`}>
                                    {/* Peak */}
                                    <div className={`px-4 py-3 ${dark ? 'bg-emerald-500/[0.04]' : 'bg-emerald-50/50'}`}>
                                        <div className={`text-[7.5px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1 ${dark ? 'text-emerald-500' : 'text-emerald-600'}`}>
                                            ✦ Peak
                                        </div>
                                        <div className={`text-xs font-black leading-none mb-1 ${dark ? 'text-emerald-300' : 'text-emerald-800'}`}>
                                            {best.label} <span className="text-[10px] font-semibold opacity-60">{best.score}/5</span>
                                        </div>
                                        <p className={`text-[9px] leading-relaxed ${dark ? 'text-gray-500' : 'text-gray-500'}`}>{best.reasoning}</p>
                                    </div>
                                    {/* Overview */}
                                    <div className="px-4 py-3">
                                        <div className={`text-[7.5px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1 ${dark ? 'text-blue-400' : 'text-blue-600'}`}>
                                            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 inline-block" /> Overview
                                        </div>
                                        <p className={`text-[9px] leading-relaxed ${dark ? 'text-gray-400' : 'text-gray-600'}`}>{data.instructor_feedback_intro}</p>
                                    </div>
                                    {/* Pivot */}
                                    <div className={`px-4 py-3 ${dark ? 'bg-amber-500/[0.03]' : 'bg-amber-50/40'}`}>
                                        <div className={`text-[7.5px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1 ${dark ? 'text-amber-400' : 'text-amber-600'}`}>
                                            <ArrowRight size={8} /> Pivot
                                        </div>
                                        <p className={`text-[9px] leading-relaxed ${dark ? 'text-gray-400' : 'text-gray-600'}`}>{data.instructor_feedback_pivot}</p>
                                    </div>
                                    {/* Next Step */}
                                    <div className="px-4 py-3">
                                        <div className={`text-[7.5px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1 ${dark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                                            <ArrowRight size={8} /> Next Step
                                        </div>
                                        <p className={`text-[9px] leading-relaxed ${dark ? 'text-gray-400' : 'text-gray-600'}`}>{data.instructor_feedback_next_step}</p>
                                    </div>
                                    {/* Focus */}
                                    <div className={`px-4 py-3 ${dark ? 'bg-red-500/[0.03]' : 'bg-red-50/40'}`}>
                                        <div className={`text-[7.5px] font-black uppercase tracking-widest mb-1.5 flex items-center gap-1 ${dark ? 'text-red-400' : 'text-red-600'}`}>
                                            ◎ Focus
                                        </div>
                                        <div className={`text-xs font-black leading-none mb-1 ${dark ? 'text-red-300' : 'text-red-800'}`}>
                                            {weakest.label} <span className="text-[10px] font-semibold opacity-60">{weakest.score}/5</span>
                                        </div>
                                        <p className={`text-[9px] leading-relaxed ${dark ? 'text-gray-500' : 'text-gray-500'}`}>{weakest.reasoning}</p>
                                    </div>
                                </div>
                            </>
                        );
                            ...
                            <div className={`${subBg} px-4 py-3 border-b ${divider2} flex items-center justify-between`}>
                                <div className="flex items-center gap-2">
                                    ...
                                    <button 
                                        ...
                                        title="Learn how Raati AI calculates these metrics"
                                        className={`p-1 rounded-md border transition-all hover:scale-110 active:scale-95 ${dark ? 'bg-blue-500/10 border-blue-500/20 text-blue-400 hover:bg-blue-500/30' : 'bg-blue-50 border-blue-100 text-blue-600 hover:bg-blue-100'}`}
                                    >
                                        <Info size={12} />
                                    </button>
                                </div>
